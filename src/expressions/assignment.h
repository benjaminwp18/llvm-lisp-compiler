...
class ASTExpressionAssignment: public ASTExpression
{
    ...
    std::unique_ptr<ASTExpressionVariable> left;
...
public:
    ...
    ASTExpressionAssignment(std::unique_ptr<ASTExpressionVariable> left, std::unique_ptr<ASTExpression> right) :
        left(std::move(left)), right(std::move(right)) {}

    // Create a new addition expression.
    // left: Value to store an expression into (has to be an L-Value).
    // right: What to store into the value on the left.
    static auto Create(std::unique_ptr<ASTExpressionVariable> left, std::unique_ptr<ASTExpression> right)
    {
